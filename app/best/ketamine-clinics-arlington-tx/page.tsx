...
export default async function CityPage() {
  ...
  const faqLd = {
    ...
    mainEntity: [
      {
        "@type": "Question",
        name: "How many ketamine clinic are in Arlington, TX?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Ketamine Clinic Directory lists 15+ ketamine clinic in Arlington, Texas. Counts change as new listings are seeded.",
        },
      },
      {
        "@type": "Question",
        name: "How do I find ketamine clinic in Arlington?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Search ketaminetherapyfinder.com and filter by Arlington. Compare listed providers, then contact the one that fits.",
        },
      },
    ],
  }
  return (
    <>
      ...
      <main className="mx-auto max-w-5xl px-4 py-10">
        <p className="text-sm text-neutral-500">Arlington, TX</p>
        <h1 className="mt-2 text-3xl font-bold tracking-tight">
          Ketamine Clinic in Arlington, TX
        </h1>
        <p className="mt-3 max-w-2xl text-neutral-600">
          15+ listed ketamine clinic in the Arlington area. Pages are generated from live directory listings — not outreach.
        </p>
          ...
          {listings.map((row: Record<string, unknown>, i: number) => (
            <li key={String(row.id || row.slug || i)} className="rounded-xl border border-neutral-200 p-4">
              ...
              <p className="mt-1 text-sm text-neutral-500">
                {String(row.city || "Arlington")}, {String(row.state || "TX")}
              </p>
            </li>
          ))}
        </ul>
