        ...
        <div className="card p-6 sm:p-8 mb-6">
          <div className="flex flex-col sm:flex-row gap-6">
            <div className="shrink-0">
              {listing.photo_url ? (<img src={listing.photo_url} alt={listing.full_name} className="h-28 w-28 rounded-2xl object-cover shadow-soft"/>) : (<div className="flex h-28 w-28 items-center justify-center rounded-2xl bg-sage-100 text-4xl font-serif font-bold text-sage-400 shadow-soft">
            ...
            <div className="flex-1">
              <div className="flex flex-wrap items-start gap-3 mb-2">
                ...
                {listing.plan_tier === 'featured' && (<span className="inline-flex items-center gap-1 rounded-full bg-amber-100 px-2.5 py-0.5 text-xs font-semibold text-amber-800 mt-1">
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {(listing.accepts_telehealth || listing.telehealth) && (<span className="inline-flex items-center gap-1 rounded-full bg-blue-50 px-3 py-1 text-xs font-medium text-blue-700">
                    <Video className="h-3 w-3"/> Telehealth
                  </span>)}
                {(listing.accepting_new_patients || listing.accepting_new_clients) && (<span className="inline-flex items-center gap-1 rounded-full bg-green-50 px-3 py-1 text-xs font-medium text-green-700">
                    <CheckCircle className="h-3 w-3"/> Accepting new patients
                  </span>)}
              </div>

              {(<div className="flex flex-wrap gap-3">
                  {listing.phone && (<a href={`tel:${listing.phone}`} className="btn-primary flex items-center gap-2 text-sm">
                      <Phone className="h-4 w-4"/>
                      {formatPhone(listing.phone)}
                    </a>)}
                  {listing.website && (<a href={listing.website} target="_blank" rel="noopener noreferrer" className="btn-secondary flex items-center gap-2 text-sm">
          ...
          {listing.bio && (<div className="mt-6 pt-6 border-t border-slate-100">
              <p className="text-charcoal-600 leading-relaxed">{listing.bio}</p>
            </div>)}
        </div>

        {listing.conditions_treated && listing.conditions_treated.length > 0 && (<div className="card p-6 mb-6">
            <h2 className="font-serif text-lg font-semibold text-charcoal-900 mb-3">Conditions Treated</h2>
            <div className="flex flex-wrap gap-2">
              {listing.conditions_treated.map((c: string) => (<span key={c} className="rounded-full bg-sage-50 px-3 py-1 text-xs font-medium text-sage-700">{c}</span>))}
            </div>
          </div>)}

        {!isClaimed && (<div className="card p-6 border-sage-200 bg-sage-50">
