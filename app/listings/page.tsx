    ...
    return (<div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="flex gap-8">
        ...
        <div className="flex-1 min-w-0">
          ...
          {isProximitySearch && city && (<div className="mb-4 rounded-lg bg-sage-50 border border-sage-200 px-4 py-3 text-sm text-sage-700">
              No ketamine clinics found in {city} — showing ketamine clinics within 25 miles of {city}.
            </div>)}

          {listings.length === 0 ? (<div className="card p-12 text-center">
              <p className="text-charcoal-500 mb-4">
                {params.tier === 'verified'
                ? "Listings are free. Try adjusting your search filters." : params.tier === 'pro'
                ? "Listings are free. Try adjusting your search filters." : 'No ketamine clinics found matching your search. Try adjusting your filters.'}
              </p>
          ...
          {totalPages > 1 && (<div className="mt-8 flex items-center justify-center gap-2">
              {page > 1 && (<Link href={`?${new URLSearchParams({ ...params, page: String(page - 1) })}`} className="btn-secondary text-sm py-2 px-4">
                  Previous
                </Link>)}
              <span className="text-sm text-charcoal-400">
                Page {page} of {totalPages}
              </span>
              {page < totalPages && (<Link href={`?${new URLSearchParams({ ...params, page: String(page + 1) })}`} className="btn-secondary text-sm py-2 px-4">
                  Next
                </Link>)}
            </div>)}
        </div>
